...
// Use a unique name to avoid redeclaration
const testDriverName = "sqlite3"

// TempFilenameSerialize creates a temporary filename for serialization tests
...
func TestSerializeDeserialize(t *testing.T) {
    t.Run("BasicSerializeDeserialize", func(t *testing.T) {
        ...
        // Confirm serialized data is non-empty
        if len(serialized) == 0 {
            t.Fatal("Serialized data is empty")
        }

        // Confirm destination database is initially empty
        ...
        // Confirm destination database has the correct data
